...
@Injectable()
export class HCLogger implements LoggerService {
    ...
    log(message: any, context?: string) {
        if (this.level >= 1)
            this.print(LogLevel.normal, message, context);
    }

    fatal(message: any, context?: string) {
        if (this.level >= 0)
            this.print(LogLevel.fatal, message, context);
    }
